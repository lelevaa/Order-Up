...
<body>
    ...
    <script>
        ...
        // Função para atualizar a lista de nomes de produtos
        ...
        // Função para atualizar a descrição, o preço e o vencimento do produto selecionado
        ...
        function filterTable() {
            const input = document.getElementById('filterDate').value;
            const table = document.getElementById('dataTable');
            const rows = table.getElementsByTagName('tr');

            for (let i = 1; i < rows.length; i++) { // Começa do índice 1 para ignorar o cabeçalho
                const cell = rows[i].getElementsByTagName('td')[0]; // Coluna "Lançamento"
                if (cell) {
                    const cellDate = cell.textContent || cell.innerText;
                    if (cellDate.includes(input)) {
        ...
        function filtrarTabelaPorLancamento() {
            ...
            for (let i = 1; i < linhas.length; i++) {
                const colunaLancamento = linhas[i].getElementsByTagName('td')[5]; // Coluna de lançamento
                if (colunaLancamento) {
                    const dataLancamento = colunaLancamento.textContent || colunaLancamento.innerText;
                    if (dataLancamento === filtroLancamento || filtroLancamento === '') {
