...
if (isset($_GET['nome'])) {
    ...
    if ($result->num_rows > 0) {
        ...
        echo "Registro não encontrado. <a href='produtosDiario.php'>Voltar</a>";
        exit();
    }
    $stmt->close();
} else {
    echo "Nome do registro não fornecido. <a href='produtosDiario.php'>Voltar</a>";
    exit();
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['editar'])) {
    ...
    // Atualiza o produto
    $sql = "UPDATE tb_produtos SET nome = ?, descricao = ?, preco = ?, categoria = ? WHERE nome = ?";
    $stmt = $conn->prepare($sql);
    ...
    if ($stmt->execute()) {
        header("Location: produtosDiario.php");
        exit();
    } else {
...
<body>
    <form action="" method="post" class="formulario">
        ...
        <a href="produtosDiario.php" class="botao">Cancelar</a>
    </form>
</body>
</html>
